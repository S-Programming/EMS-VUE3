...
<div class="block-content block-content-full">
    <table id="userCheckinHistoryTbl"
        ...
        @if(isset($user_history) && !empty($user_history))
            @foreach($user_history as $data)
                <tr>
                <!-- <td>{{$data->user_id??''}}</td> -->
                    <td data-sort="{{$data->id}}">{{$data->checkin??''}}</td>
                    <td>{{$data->checkout ??''}}</td>
                ...
                <!-- <td>{!!$data->done_today??'' !!}</td> -->
                    <td class="text-center">
                        <button class="btn btn-primary"
                                onclick="commonAjaxModel('report/today/modal', {{$data->id}})"><span
                                class="text-center">Summary</span></button>
                    </td>
                    @can('isAdmin')
                        <td>
                            <button class="btn btn-info"
                                    onclick="commonAjaxModel('edit/checkin/user/modal', {{$data->id}})">
                                <i class="fa fa-edit"></i></button>
                            <button class="btn btn-danger"
                                    onclick="commonAjaxModel('delete/checkin/user/modal',{{$data->id}})">
